...
split_sample <-
  groupdata2::partition(
    all_df %>% dplyr::filter(source == "Rhön"),
    p = 0.8,
    cat_col = "field_id",
    force_equal = FALSE,
    list_out = F
...
save(pls_n, pls_adf, rf_n,rf_adf, gp_n, gp_adf, svm_n, svm_adf, file = "./output/models/pls_rfr_gpr_svmr_for_n_adf_dsb.RData")
...
write.csv(op_df, "./output/models/plsr_rfr_gpr_svmr_op_dsb.csv", row.names = FALSE)
