...
op_df <- read.csv("./output/models/plsr_rfr_gpr_svmr_nnr_op.csv")
...
post_resample_nn_n <- postResample(op_df$nn_n_pred, op_df$n)
post_resample_nn_adf <- postResample(op_df$nn_adf_pred, op_df$adf)
...
nn_n_plot <- ggplot(op_df, 
  ...
  labs(x="Predicted", 
       y="Observed",
       title = "NNR",
       caption=paste("R² = ", round(post_resample_nn_n[2], 2), ",   ",
                     "rRMSEP = ", round(post_resample_nn_n[1]/mean_n, 2)*100, 
                     " %", sep = "")) +
  theme(axis.text = element_text(size=13),
        axis.title = element_text(size=14),
        legend.title = element_text(size=13, face="bold"),
...
n_plots <- cowplot::plot_grid(pls_n_plot, rf_n_plot, gp_n_plot, svm_n_plot, nn_n_plot, 
...
nn_adf_plot <- ggplot(op_df, 
                       aes(x=nn_adf_pred, y=adf)) +
  geom_point(size = 2, alpha  = 0.8, aes(col=source))  + 
  stat_smooth(method=lm, se = FALSE, linetype = "twodash") + 
  geom_abline(mapping = NULL, data = NULL, slope = 1, intercept = 0, 
              linetype = "twodash", color = "black") +
  ...
  labs(x="Predicted", 
       y="Observed",
       title = "NNR",
       caption=paste("R² = ", round(post_resample_nn_adf[2], 2), ",   ",
                     "rRMSEP = ", round(post_resample_nn_adf[1]/mean_adf, 2)*100, 
                     " %", sep = "")) +
  theme(axis.text = element_text(size=13),
        axis.title = element_text(size=14),
        legend.title = element_text(size=13, face="bold"),
...
adf_plots <- cowplot::plot_grid(pls_adf_plot, rf_adf_plot, gp_adf_plot, svm_adf_plot, nn_adf_plot,
                   nrow = 2, ncol = 3)
...
taylor.diagram(op_df$n, op_df$nn_n_pred, add = TRUE, col="forestgreen", pch = 19, pcex = 2)
lpos <- 1.5*sd(op_df$n)
legend(0.85, 1.0, legend=c("Observed", "PLSR", "RFR", "GPR", "SVMR", "NNR"),
       pch = 19, cex = 1.5, col = c("red", "#9449d2", "#B1740F", "blue", "#08585A", "forestgreen"))
par(oldpar)
dev.off()
...
taylor.diagram(op_df$adf, op_df$nn_adf_pred, add = TRUE, col="forestgreen", pch = 19, pcex = 2)
lpos <- 1.5*sd(op_df$adf)
legend(5.2, 5.3, legend=c("Observed","PLSR","RFR","GPR","SVMR", "NNR"),
       pch = 19, cex = 1.5, col = c("red", "#9449d2", "#B1740F", "blue", "#08585A", "forestgreen"))
par(oldpar)
dev.off()
...
op_df$nn_n_delta <- op_df$n - op_df$nn_n_pred
op_df$nn_adf_delta <- op_df$adf - op_df$nn_adf_pred
...
res_vs_fid_n <- ggplot(data = op_df %>% 
  ...
  jcolors::scale_fill_jcolors("pal7",
                              name = "Model", 
                              labels = c("PLSR", "RFR", "GPR", "SVMR", "NNR")) +
