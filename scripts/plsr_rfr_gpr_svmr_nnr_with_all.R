...
nn_tunegrid_n <- expand.grid(.decay = c(0.5), .size = c(7))
nn_tunegrid_adf <- expand.grid(.decay = c(0.5), .size = c(7))
...
save(pls_n, pls_adf, rf_n,rf_adf, gp_n, gp_adf, svm_n, svm_adf, nn_n, nn_adf, 
     file = "./output/models/pls_rfr_gpr_svmr_nnr_for_n_adf.RData")
...
### NNR
nn_n_pred <- predict(nn_n , test_df[, estimators])
postResample(pred = nn_n_pred, obs = test_df$n)
nn_adf_pred <- predict(nn_adf , test_df[, estimators])
postResample(pred = nn_adf_pred, obs = test_df$adf)
...
op_df <- cbind(test_df[-(estimators)], pls_n_pred, pls_adf_pred,
               ...
               nn_n_pred, nn_adf_pred)

write.csv(op_df, "./output/models/plsr_rfr_gpr_svmr_nnr_op.csv", row.names = FALSE)
