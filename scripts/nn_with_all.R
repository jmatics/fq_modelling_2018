###############################################
# 02.03.2018
# Forage quality modelling with NueralNet
# JW
###############################################
...
## 3.1 NNR
metric <- "RMSE"
#tunegrid <- data.frame(layer1 = 1:5, layer2 = 1:3, layer3 = 1:2)
tunegrid <- expand.grid(.decay = c(0.5, 0.2, 0.1), .size = c(5, 6, 7, 8))
...
save(nn_all_n, nn_all_adf, file = "./output/models/nnr_model_for_N_ADF.RData")

### Visulaise calibrated models

plot(varImp(nn_all_n), 10, main = "N_NNR")
plot(varImp(nn_all_adf), 10, main = "ADF_NNR")


nn_all_compare <- resamples(list("N_NNR" = nn_all_n,
                                  "ADF_NNR" = nn_all_adf))
summary(nn_all_compare)


bwplot(nn_all_compare, scales=list(tck=c(1,0), x=list(cex=1.5), y=list(cex=1.5)))
dotplot(nn_all_compare, scales=list(tck=c(1,0), x=list(cex=1.5), y=list(cex=1.5)))
...
obs_pred_plot(nn_all_n, test_df, 
              estimators, targetN, plot = TRUE, 
              title = "N (%) estimation model with NNR")


obs_pred_plot(nn_all_adf, test_df, 
