...
## 3.1 MARS
metric <- "RMSE"
tunegrid <- expand.grid(degree = c(1:4), nprune = c(2:20))
...
save(mars_all_n, mars_all_adf, file = "./output/models/mars_model_for_N_ADF.RData")
timestamp()
### Visulaise calibrated models

plot(varImp(mars_all_n), 10, main = "N_MARS")
plot(varImp(mars_all_adf), 10, main = "ADF_MARS")


mars_all_compare <- resamples(list("N_MARS" = mars_all_n,
                                 "ADF_MARS" = mars_all_adf))
summary(mars_all_compare)


bwplot(mars_all_compare, scales=list(tck=c(1,0), x=list(cex=1.5), y=list(cex=1.5)))
dotplot(mars_all_compare, scales=list(tck=c(1,0), x=list(cex=1.5), y=list(cex=1.5)))
...
obs_pred_plot(mars_all_n, test_df, 
              estimators, targetN, plot = TRUE, 
              title = "N (%) estimation model with MARS")


obs_pred_plot(mars_all_adf, test_df, 
              estimators, targetADF, plot = TRUE, 
              title = "ADF (%) estimation model with MARS")
